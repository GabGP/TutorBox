...
def test_list_devices_empty_and_populated(staff_db, client):
    ...
    # Empty fleet
    resp = client.get("/api/v1/staff/devices", headers=headers)
    assert resp.status_code == 200
    assert resp.json() == {"devices": []}

    # Register 2 devices
    client.post("/api/v1/staff/devices", json={"device_id": "1"}, headers=headers)
    client.post(
        "/api/v1/staff/devices", json={"device_id": "ESP32_02"}, headers=headers
    )
...
def test_register_device_success_and_conflict(staff_db, client):
    ...
    # Success
    resp = client.post(
        "/api/v1/staff/devices", json={"device_id": "CLICKER_01"}, headers=headers
    )
    ...
    resp3 = client.post(
        "/api/v1/staff/devices", json={"device_id": ""}, headers=headers
    )
...
def test_assign_device_success_and_reassignment(staff_db, client):
    ...
    client.post("/api/v1/staff/devices", json={"device_id": "1"}, headers=headers)
    client.post("/api/v1/staff/devices", json={"device_id": "2"}, headers=headers)

    # Assign clicker 1 to student1
    resp = client.post(
        "/api/v1/staff/devices/1/assign", json={"user_id": s1_id}, headers=headers
    )
    assert resp.status_code == 200
    assert resp.json() == {
        "device_id": "1",
    ...
    resp2 = client.post(
        "/api/v1/staff/devices/2/assign", json={"user_id": s1_id}, headers=headers
    )
    assert resp2.status_code == 200

    list_resp = client.get("/api/v1/staff/devices", headers=headers)
    ...
    resp3 = client.post(
        "/api/v1/staff/devices/2/assign", json={"user_id": s2_id}, headers=headers
    )
...
def test_assign_device_error_cases(staff_db, client):
    ...
    # Missing device (404)
    resp = client.post(
        "/api/v1/staff/devices/999/assign", json={"user_id": s1_id}, headers=headers
    )
    ...
    resp2 = client.post(
        "/api/v1/staff/devices/1/assign", json={"user_id": 9999}, headers=headers
    )
    ...
    resp3 = client.post(
        "/api/v1/staff/devices/1/assign", json={"user_id": t1_id}, headers=headers
    )
    assert resp3.status_code == 422
...
def test_unassign_device(staff_db, client):
    ...
    resp = client.post("/api/v1/staff/devices/1/unassign", headers=headers)
    assert resp.status_code == 200
    ...
    resp2 = client.post("/api/v1/staff/devices/1/unassign", headers=headers)
    assert resp2.status_code == 200

    # Unassign non-existent device (404)
    resp3 = client.post("/api/v1/staff/devices/999/unassign", headers=headers)
...
def test_delete_device(staff_db, client):
    ...
    # Delete existing
    resp = client.delete("/api/v1/staff/devices/1", headers=headers)
    assert resp.status_code == 200
    ...
    resp2 = client.delete("/api/v1/staff/devices/1", headers=headers)
    assert resp2.status_code == 404
...
def test_device_cascade_on_user_deletion(staff_db, client):
    ...
    del_resp = client.delete(f"/api/v1/staff/users/{s1_id}", headers=headers)
...
def test_device_rbac_and_pin_rotation_gating(staff_db, client):
    ...
    assert (
        client.get("/api/v1/staff/devices", headers=student_headers).status_code == 403
    )
    assert (
        client.post(
            "/api/v1/staff/devices", json={"device_id": "1"}, headers=student_headers
        ).status_code
        == 403
    )
    ...
    reset_resp = client.post(
        f"/api/v1/staff/users/{admin_id}/reset-pin", headers=admin_headers
    )
    ...
    login_resp = client.post(
        "/api/v1/auth/login", json={"username": "admin1", "pin": temp_pin}
    )
    ...
    assert client.get("/api/v1/staff/devices", headers=gated_headers).status_code == 403
...
def test_device_audit_trail_recorded(staff_db, client):
    ...
    client.post("/api/v1/staff/devices/1/unassign", headers=headers)
    client.delete("/api/v1/staff/devices/1", headers=headers)

    audit_resp = client.get("/api/v1/staff/audit-logs", headers=headers)
